	...
	return Gonrin.ModelView.extend({
		...
		tools: [{
			...
			buttons: [{
				...
				},
				{
					...
					command: function () {
						var self = this;
						var filename = "kehoach_bcc_caphuyen_"+self.model.get("nambaocao")+"_ky_"+self.model.get("kybaocao");
		...
		render: function () {
			...
			self.$el.find("#add_dmhoatdong").unbind("click").bind("click", function(event) {
				var dmHoatDongDialog = new DMHoatDongSelectView({"viewData":{"loai_hoatdong":"huyen"}});
		...
		setDefaultData: function() {
			...
			self.model.set("tuyendonvi", "huyen");
		},
		...
		renderDanhSach: function(danhsachhoatdong) {
            ...
            <tr>
                <td colspan="5" class="text-left" style="color: red; font-weight: bold;">Hoạt động cấp huyện</td>
                <td></td>
                <td></td>
                <td></td>
            </tr>`);
			...
			if(danhsachhoatdong === null){
				$.ajax({
		    		...
		    		data: {"q": JSON.stringify({"filters": {"loai_hoatdong":{"$eq":"huyen"}}, "order_by":[{"field": "nganh_id", "direction": "asc"}]})},
					type: "GET",
					success: function(data) {
